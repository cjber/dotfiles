...
if ! systemctl --user is-enabled gdrive.timer &>/dev/null; then
    echo '{"text":"gd off","class":"disconnected","tooltip":"Google Drive sync is disabled"}'
    exit 0
fi

if pgrep -x rclone >/dev/null 2>&1; then
    echo '{"text":"gd syncing","class":"syncing","tooltip":"Google Drive sync in progress"}'
    exit 0
fi

if systemctl --user is-failed gdrive.service &>/dev/null; then
    echo '{"text":"gd failed","class":"warning","tooltip":"Google Drive last sync failed"}'
    exit 0
fi
...
echo "{\"text\":\"gd\",\"class\":\"connected\",\"tooltip\":\"${tooltip}\"}"
