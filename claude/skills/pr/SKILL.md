...
Deliver a reviewed, green PR — always opened ready-for-review, never a draft, and always **one PR per repo per session**. Claude runs on Opus throughout; Codex carries the bulk of the implementation with an Opus arm working a disjoint slice alongside it for speed.
...
- **One PR per repo, always.** If this session already opened a PR for this repo, push to that same branch and update it — never open a second.
...
Terminate only when all required checks are green and no actionable review thread remains. Leave the single PR ready-for-review (never draft) and unmerged.
