...
- **HARD RULE — staging PR descriptions mirror the complete main diff.** For every staging-targeted draft PR, generate and verify the full body from `origin/main...HEAD`, not merely `origin/cb/staging...HEAD`. Re-read that diff and refresh the body after every pushed commit and immediately before handoff, including staging-only changes that will eventually promote to main. A stale or partial body blocks completion.
