...
**HARD RULE — every out-of-scope finding becomes a tracked issue, never a silent drop.** Any time the plan, review, or implementation surfaces something that *should* change but you decide **not** to handle in this PR — a lever cut on evidence, a design that doesn't survive the code/data, a follow-up refactor, an assumption proven wrong — file a GitHub issue for it (with the evidence that drove the cut) and link it from the PR body, in the same session. Deferring is fine; forgetting is not. A carve-out with no issue is a dropped thread; the issue is what makes "we won't do this now" a decision instead of a loss. This applies whether the finding appears at plan time or mid-build.
...
**HARD RULE — draft provenance is a release boundary.** A PR or commit explicitly marked draft, experimental, spike, prototype, or not approved for staging MUST NOT be merged, bundled, or cherry-picked into `cb/staging`, even when CI is green, reviewers approved it, a feature flag defaults off, or clients hide the surface. Verify release authorization from authoritative PR metadata and author comments before incorporating external work. If draft work is discovered in staging, revert it explicitly; feature-disabling is not removal.
...
Terminate only when ALL of these hold: required checks green; no actionable review thread remains; the **final `cli verify` pass over the final diff** (above) is green or every skipped surface is explicitly justified (sandbox-blocked / infra-only); the branch is **up to date with `origin/cb/staging`** (fetch + merge staging and re-run the full gate if it drifted; `gh pr view` shows base `cb/staging` and `MERGEABLE`/`CLEAN`, not `CONFLICTING`/`BEHIND`); this repo has exactly **one** task PR (this one); and **every scope cut / deferred finding / disproven assumption from this session has a filed, PR-linked issue** (the step-1 hard rule) — no carve-out left only in your head or the chat log. Leave it ready-for-review and unmerged. After approved task PRs land, `$dev` owns composed `/dev` verification and the eventual `cb/staging` → `main` promotion PR.
