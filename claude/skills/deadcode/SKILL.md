...
Delete only `(reachability ∪ scanner) − executed − roots − cross_repo_allowlist`, then **iterate to a fixpoint**: removing one island orphans the next. *(nebula: `uv run sift deadcode` runs the vulture symbol pass + the AST module-reachability pass with these roots + the parallax allowlist already wired; `[tool.vulture] ignore_decorators` suppresses framework symbols structurally. It groups hits by file largest-first, and labels each as fully-dead, test-only (delete the test too), or LIVE-via-a-`scripts`/`alembic`/`dev`-caller. `-p <path>` scopes it to one file or directory.)*
